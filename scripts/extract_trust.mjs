...
async function run() {
  ...
  const data = await page.evaluate(() => {
    // Look for the "Trusted by teams at" section
    const el = [...document.querySelectorAll('div')].find(d => d.innerText.includes('Trusted by teams at'));
    if (!el) return { error: 'TrustSection not found' };
  ...
  fs.writeFileSync(path.join(ROOT, 'docs/research/components/TrustSection.json'), JSON.stringify(data, null, 2));
  console.log('TrustSection data saved.');
